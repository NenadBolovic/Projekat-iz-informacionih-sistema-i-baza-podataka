...
import {getAnswersByGroup} from '../controllers/answercontroller.js';
import { UnauthorizedError,BadRequestError,ForbiddenError,InternalServerError} from '../errors.js';

describe('Get answers of form',()=>{
    let req,res,next;
    let mockAxios, mockGetAnswersByFIDQID,getAnswersByGroupTest;

    beforeEach(async ()=>{
        req={
            header: sinon.stub(),
            params: {formId: 123,questionOrderId:1}
        };
        ...
        getAnswersByGroupTest=await getAnswersByGroup({axios: mockAxios,getAnswersByFIDQID: mockGetAnswersByFIDQID});
    ...
    it('Access denied, no token provided.', async()=>{
        ...
        await getAnswersByGroupTest(req,res,next);
    ...
    it('user is the form owner return answers', async () => {
        ...
        mockAxios.post.resolves({ data: { user: { userId: '123' } } });
        mockAxios.get.resolves({ data: { form: { authId: '123', collaborators: [], observers: [] } } });
        mockGetAnswersByFIDQID.resolves([{ answer: 'Test Answer' }]);
    ...
    it('Internal error should return 500 error when getAnswersByFIDQID fails', async () => {
        ...
        mockAxios.post.resolves({ data: { user: { userId: '123' } } });
        mockAxios.get.resolves({ data: { form: { authId: '123', collaborators: [], observers: [] } } });
        mockGetAnswersByFIDQID.rejects(new InternalServerError('Database error occurred'));
